...
export async function GET() {
  ...
  const [vendor, ledger, items, monthLines] = await Promise.all([
    db.vendor.findUnique({ where: { id: vendorId }, select: { id: true, code: true, businessName: true, contactName: true, email: true, commissionPercent: true, mustChangePassword: true, acceptsPreorders: true, acceptsRequests: true, publicBlurb: true, allowSelfCheckout: true, cardLast4: true, contracts: { orderBy: { createdAt: "desc" }, take: 1, select: { id: true, status: true, vendorSignedAt: true } } } }),
